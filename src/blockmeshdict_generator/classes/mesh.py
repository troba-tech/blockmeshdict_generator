...
class Mesh():
    ...
    @property
    def boundaries(self):
        # Block contains boundaries according to the example in __init__()
        # this method collects all faces for a boundary name from all blocks
        # (a format ready for jinja2 template)

        # collect all boundary names
        boundary_names = []
        for block in self.blocks:
            boundary_names += list(block.boundaries.keys())

        # keep the boundaries unique
        boundary_names = list(set(boundary_names))

        # create a dict with all boundaries
        boundaries = { name: [] for name in boundary_names }

        # gather all faces of all blocks
        for block in self.blocks:
            for boundary_name in boundary_names:
                boundaries[boundary_name] += block.get_faces(boundary_name)
    ...
    def set_boundary_type(self, boundary_name, boundary_type):

        #TO DO need to check that boundary_type in ['wall', 'patch',empty']
        # Function not tested yet

         self.boundaries_type[boundary_name] = boundary_type

    def get_boundary_type(self, boundary_name):
        
        # Function not tested yet

        if boundary_name in self.boundaries_type:
            return self.boundaries_type[boundary_name]
        else:
            return "patch"
    ...
    def write(self, template_path, output_path, context=None):
        ...
        context = {
            ...
            'boundaries': { 
                boundary_name: {
                    "faces": faces,
                    "type": self.get_boundary_type(boundary_name)
                } for boundary_name, faces in self.boundaries.items()},            
        }
