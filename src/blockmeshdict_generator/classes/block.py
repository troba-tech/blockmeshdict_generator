...
class Block():
    ...
    # a more intuitive and quicker way to set boundaries,
    ...
    ### Information
    ###
    def get_faces(self, boundary):
        if boundary not in self.boundaries:
            return []

        sides = self.boundaries[boundary]
        faces = []
    ...
    ### Manipulation
    ###
    def set_boundary(self, sides, boundary_name):
        """ assign one or more block faces (self.face_map)
        to a chosen boundary name """
        # see boundaries: an example in __init__()
        ...
        self.boundaries[boundary_name] += sides
